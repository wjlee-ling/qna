...
4-2. 최종 답변을 만들 때 근거가 된 내용이 [참고 자료]에 있었으면 해당 내용(파란색 칸)을 복사하여 작업시트의 [출처]에 기입. 여러 참고자료를 참고하였으면 contrl+엔터로 구별하여 모두 기입
...
4-5.그래도 답변을 만들기 어려우면 그냥 해당 행을 넘어가기

1. [참고 자료]를 이용하여 해당 대표질의/유사질의에 답이 되는 최종 답변을 만들었다면 해당 대표질의를 갖고 있는 행들의 [답변]에 복사 붙여넣기. 다만 [유사질의]와 문맥이 맞지 않는 경우 해당 [유사질의]가 있는 행은 답변을 채우지 않고 넘어가기.
2. 과정 (5)에서 새로 답변을 채운 모든 행들의 [최종작업일]을 수정. 
3. 과정 (5)에서 새로 답변을 채운 모든 행들의 질문/답변이 전기차/ev “특화” 질문/답변이라면 [ev] TRUE로 기입\
"""
    )
